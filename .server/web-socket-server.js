...
module.exports = class WebSocketServer {
    ...
    create() {//服务器发来消息 服务器传进来 conn
        this.server = ws.createServer((conn) => {
            console.log('New connection');
            conn.on("text", (str) => {
                console.log("收到的信息为:" + str);
                let msg = JSON.parse(str);
                this._pageMap.set(msg.userId, msg.type);//pageMap
                if (msg.type === 'get-conversations') {//这是啥
                    conn.sendText(JSON.stringify({//sendText 函数在下面
                        type: msg.type,
                    ...
                    let temp;
                    this.getSingleHistoryMsg(msg.userId, msg.friendId).forEach(item => {                    //item  
                    ...
                    conn.sendText(JSON.stringify({
                        type: msg.type,
                        friends: this._users.filter(item => item.userId !== msg.userId)//过滤掉非好友？
                    }));
    ...
    sendText(conn, msg, cbOk) {
        if (this._pageMap.get(msg.friendId) !== 'get-conversations') {
            ...
            const msgSendFinally = JSON.stringify({type, content, duration,timestamp});                         /*参数*/        /*函数*/
            conn.sendText(msgSendFinally, () => {
                cbOk && cbOk(msgSendFinally);//cbOK
            });
        } else {
    ...
    getConversation(userId) {
        ...
        if (item.msgUserId === userId) {//本人会话
            conversations.push({
                friendId: item.friendId,
                msgUserId: item.msgUserId,//消息所有人的id
